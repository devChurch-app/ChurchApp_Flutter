...
class CommentsModel with ChangeNotifier {
  ...
  Future<void> fetchComments() async {
    try {
      final response = await http.post(ApiUrl.loadcomments,
          body: jsonEncode({
  ...
  Future<void> constructComment(String content) async {
    try {
      ...
      print(data.toString());
      final response =
          await http.post(ApiUrl.makecomment, body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
  ...
  Future<void> deleteComment(int commentId, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(ApiUrl.deletecomment,
  ...
  Future<void> editComment(int id, String content, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response =
          await http.post(ApiUrl.editcomment, body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
        Map<String, dynamic> res = json.decode(response.body);
  ...
  Future<void> reportComment(int commentId, int position, String reason) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(ApiUrl.reportcomment,
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
