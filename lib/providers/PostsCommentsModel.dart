...
class PostsCommentsModel with ChangeNotifier {
  ...
  Future<void> fetchComments() async {
    try {
      final response = await http.post(ApiUrl.loadpostcomments,
          body: jsonEncode({
            "data": {"id": 0, "post": post}
          }));
  ...
  Future<void> constructComment(String content) async {
    try {
      ...
      print(data.toString());
      final response = await http.post(ApiUrl.makepostcomment,
  ...
  Future<void> deleteComment(int commentId, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(ApiUrl.deletepostcomment,
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
  ...
  Future<void> editComment(int id, String content, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(ApiUrl.editpostcomment,
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
  ...
  Future<void> reportComment(int commentId, int position, String reason) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(ApiUrl.reportpostcomment,
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
